...
add_action( 'init', array( 'Upomahin_Nav_Menu_Custom_Field', 'setup' ) );

class Upomahin_Nav_Menu_Custom_Field {
	...
	static function setup() {
		...
		$new_fields = apply_filters( 'upomahin_nav_menu_item_additional_fields', array() );
		if ( empty($new_fields) )
			return;
		self::$options['fields'] = self::get_fields_schema( $new_fields );

		add_filter( 'wp_edit_nav_menu_walker', function () {
			return 'Upomahin_Walker_Nav_Menu_Edit';
		});
...
class Upomahin_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
	function start_el(&$output, $item, $depth, $args) {
		$item_output = '';
		parent::start_el($item_output, $item, $depth, $args);
		// Inject $new_fields before: <div class="menu-item-actions description-wide submitbox">
		if ( $new_fields = Upomahin_Nav_Menu_Custom_Field::get_field( $item, $depth, $args ) ) {
...
// Somewhere in config...
add_filter( 'upomahin_nav_menu_item_additional_fields', 'upomahin_menu_item_additional_fields' );
function upomahin_menu_item_additional_fields( $fields ) {
	$fields['icon'] = array(
		'name' => 'icon',
		'label' => __('Icon', 'xteam'),
